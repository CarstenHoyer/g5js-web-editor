...
class PreviewFrame extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (isPlaying && previewIsRefreshing) {
      this.renderSketch();
      return;
    }

    // if user switches textoutput preferences
    if (isAccessibleOutputPlaying !== prevProps.isAccessibleOutputPlaying) {
      this.renderSketch();
      return;
    }

    if (textOutput !== prevProps.textOutput) {
      this.renderSketch();
      return;
    }

    if (gridOutput !== prevProps.gridOutput) {
      this.renderSketch();
      return;
    }

    if (soundOutput !== prevProps.soundOutput) {
      this.renderSketch();
      return;
    }

    if (fullView && files[0].id !== prevProps.files[0].id) {
      this.renderSketch();
    }
  ...
  handleConsoleEvent(messageEvent) {
    ...
    if (Array.isArray(messageEvent.data)) {
      const decodedMessages = messageEvent.data.map((message) => Object.assign(Decode(message.log), {
  ...
  mergeLocalFilesAndEditorActiveFile() {
    const { files: propFiles, cmController } = this.props
    const files = propFiles.slice();
    if (cmController.getContent) {
      const activeFileInEditor = cmController.getContent();
  ...
  injectLocalFiles() {
    const {
      htmlFile: htmlFileProps,
      textOutput,
      gridOutput,
      soundOutput
    } = this.props
    const htmlFile = htmlFileProps.content;
    let scriptOffs = [];
      ...
      if (jsFileString.match(MEDIA_FILE_QUOTED_REGEX)) {
        ...
        if (resolvedFile) {
          if (resolvedFile.url) {
            ...
            setBlobUrl(resolvedFile, blobURL);
